/* ShaderBackground: port vanilla dari komponen React "Static Radial Gradient"
   (21st.dev Shader Builder). Diadaptasi dari Paper Shaders (Apache-2.0):
   https://shaders.paper.design/static-radial-gradient

   Sumber GLSL-nya dipertahankan apa adanya. Yang diterjemahkan hanya lapisan
   lifecycle-nya (useEffect/useRef -> fungsi biasa), karena proyek ini vanilla
   HTML/CSS/JS tanpa build step. Komponen aslinya memang tanpa dependency,
   jadi tidak ada yang hilang dari hasil akhirnya.

   Kalau WebGL tidak tersedia, fungsi ini keluar diam-diam dan .hero-glow
   yang lama tetap tampil sebagai fallback CSS. */
...
vec3 shade(vec2 uv, vec2 p, float t) {
  vec2 center = (hash22(vec2(u_seed, 4.7)) - 0.5) * u_intensity;
  float radius = length(p - center);
  float ripple = (fbm(p * 2.0 + u_seed) - 0.5) * (1.0 - u_paramA) * 0.35;
  return palette(clamp(radius * mix(1.9, 0.65, u_paramA) + ripple, 0.0, 1.0));
}
...
`;
  ...
  /* Warna diselaraskan ke palet EQUAL: hitam -> pink gelap -> #ffacdf -> #ffd7ef.
     Preset asli 21st.dev memakai ungu/magenta (#3A0CA3, #F72585) yang bukan
     bagian dari palet ini. Untuk kembali ke preset asli, ganti empat baris
     colors di bawah dengan:
       [0.031, 0.020, 0.102], [0.227, 0.047, 0.639],
       [0.969, 0.145, 0.522], [1, 0.839, 1] */
  const UNIFORMS = {
    colors: [
      [0, 0, 0],
      [0.16, 0.055, 0.115],
    ...
    offsetX: 0.0,
    offsetY: 0.0,
    /* Preset asli "Static" memakai drift & timeScale 0, jadi diam total.
       Dua nilai di bawah yang menghidupkannya: drift menggeser medan warna,
       timeScale mengatur kecepatannya. Sengaja lambat (satu siklus penuh
       sekitar 45 detik) supaya terasa bernapas, bukan bergoyang. */
    drift: 0.14,
    cursorEnabled: false,
    cursorEffect: 2.0,
    cursorStrength: 0.65,
  ...
  function initShaderBackground(canvas) {
    ...
    const flatColors = [];
    UNIFORMS.colors.forEach((c) => flatColors.push(c[0], c[1], c[2]));
    ...
    /* Shader ini beranimasi terus-menerus, dan media query CSS tidak bisa
       menghentikan requestAnimationFrame. Jadi preferensi reduced-motion
       dijaga di sini: waktu dibekukan, gradiennya digambar sekali lalu diam. */
    const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)');
    let motionAllowed = !reduceMotion.matches;
    const effectiveTimeScale = () => (motionAllowed ? UNIFORMS.timeScale : 0);
    const isTimeAnimated = () => Math.abs(effectiveTimeScale()) > 0.0001;
    ...
    function render(now) {
      ...
      if (isTimeAnimated() || pointerSettling) {
        requestRender();
      } else {
        lastNow = null;
      }
